...
const ParticipationTickets = ({participation, className}: {participation: ProductsProps[] | string, className?: string}) => {

  if(typeof participation === 'string') return;

  const isPatreon = participation.some(product => product.category === 'patreon')
  const products = participation.filter(product => product.category === 'week')

  const weeks = [false, false, false, false]
  products.forEach(product => {
    if (product.name.toLowerCase().includes('week 1')) weeks[0] = true
    if (product.name.toLowerCase().includes('week 2')) weeks[1] = true 
    if (product.name.toLowerCase().includes('week 3')) weeks[2] = true
    if (product.name.toLowerCase().includes('week 4')) weeks[3] = true
  })

  return (
    <TableCell className={className}>
      <CellControl value={participation}>
...
const Ticket = ({week, isPatreon}: {week: boolean, isPatreon: boolean}) => {
  if(isPatreon && week) return(
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M2 4.66732V6.00065C2.53043 6.00065 3.03914 6.21136 3.41421 6.58644C3.78929 6.96151 4 7.47022 4 8.00065C4 8.53108 3.78929 9.03979 3.41421 9.41487C3.03914 9.78994 2.53043 10.0007 2 10.0007V11.334C2 12.0673 2.6 12.6673 3.33333 12.6673H12.6667C13.0203 12.6673 13.3594 12.5268 13.6095 12.2768C13.8595 12.0267 14 11.6876 14 11.334V10.0007C13.4696 10.0007 12.9609 9.78994 12.5858 9.41487C12.2107 9.03979 12 8.53108 12 8.00065C12 7.47022 12.2107 6.96151 12.5858 6.58644C12.9609 6.21136 13.4696 6.00065 14 6.00065V4.66732C14 4.3137 13.8595 3.97456 13.6095 3.72451C13.3594 3.47446 13.0203 3.33398 12.6667 3.33398H3.33333C2.97971 3.33398 2.64057 3.47446 2.39052 3.72451C2.14048 3.97456 2 4.3137 2 4.66732Z"
      fill={week ? "#16B74A" : ''} stroke={week ? "white" : '#94A3B8'} strokeLinecap="round" strokeLinejoin="round"/>
