...
export default function MergeEmails() {
  ...
  const handleInitiateLink = async () => {
    ...
    if (profile?.primary_email && email.toLowerCase() === profile.primary_email.toLowerCase()) {
    ...
    try {
      ...
      if (response?.status === 201) {
        ...
        const errorMessage =
          response?.data?.detail || "Failed to send verification code. Please try again."
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        const errorMessage = response?.data?.detail || "Invalid verification code"
        setError(errorMessage)
        setVerificationCode(["", "", "", "", "", ""])
      ...
      const errorMessage =
        err?.response?.data?.detail || err?.message || "Invalid verification code"
      setError(errorMessage)
      setVerificationCode(["", "", "", "", "", ""])
      codeInputRefs.current[0]?.focus()
    } finally {
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (response?.status === 201) {
        ...
        const errorMessage =
          response?.data?.detail || "Failed to resend verification code. Please try again."
        setError(errorMessage)
      }
    } catch (err: any) {
      const errorMessage =
  ...
  const renderSuccessStep = () => (
    <>
      <div className="space-y-4 mt-4 text-center">
        ...
        <h3 className="text-xl font-semibold text-gray-900">Accounts Linked Successfully!</h3>
        <p className="text-sm text-gray-600">
          Your accounts are now linked. Your profile now shows combined data from both accounts.
        </p>
        <div className="pt-4">
          <Button
            onClick={handleClose}
          ...
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-[#64748b]">
                Connect multiple emails to see combined stats across all your activities. You can
                also do this in settings.
              </p>
            </div>
          </div>
